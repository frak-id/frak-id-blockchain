...
(async () => {
  try {
    console.log("Start to update our contracts to v1.0.5");
    ...
    const nameToAddresses = [{ name: "ContentPool", address: addresses.contentPool }];
    await updateContracts(nameToAddresses);

    console.log("Finished to update our contracts to v1.0.5");
  } catch (e: any) {
