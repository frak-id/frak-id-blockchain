...
contract RewarderPayTest is RewarderTestHelper {
     ...
     */
    function test_payUser() public withFrkToken(rewarderAddr) prankExecAsDeployer {
        uint16[] memory listenCounts = new uint16[](1);
        listenCounts[0] = 10;
        rewarder.payUser(address(1), 1, contentId.asSingletonArray(), listenCounts);
    }

    function testFuzz_payUser(uint16[] memory listenCounts) public withFrkToken(rewarderAddr) prankExecAsDeployer {
